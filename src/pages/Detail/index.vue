...
export default {
  ...
  methods: {
   ...
   async toSuccess(){
    // 先发请求, 判定是否成功, dispatch函数传参时,如果有多个参数,必须使用对象的形式
    // 两个参数, 商品的id , 商品的数量
    try { 
      let result = await this.$store.dispatch('addorUpdateShopCart',{skuId:this.skuInfo.id,skuNum:this.skuNum})
      alert(result)
      // 在添加购物车成功跳转到 添加购物车页面之前, 把响应的商品信息储存在sessionStorage中, 用来在 购物车成功页面去使用
      sessionStorage.setItem('SKUID_KEY',JSON.stringify(this.skuInfo))
      this.$router.push(`/addcartsuccess?skuNum=${this.skuNum}`) 
      // 如果添加购物车成功,那么就跳转到添加购物成功的页面,并通过query参数 携带对应商品的数量
    } catch (error) {
      alert(error.message)  // 这里的message, 就是我们 new Error中的错误
    }
    // 成功 
    // 失败
   },
